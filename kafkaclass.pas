...
type
  ...
  public
    ...
    _MaxConsumerMessageSize: Integer;
...
var MyMessage: String;
    MyKey: String;
begin
  _MaxConsumerMessageSize := InMaxMessageLength;
  _BStop := False;
        ...
        then begin
          ...
          if _Rkmessage^.err <> 0 then begin
            if _OnKafkaMessageErr <> nil then begin
              ...
              if _MaxConsumerMessageSize = 0 then begin
                KafkaWriteStatus('Received from topic: ' + String(PChar(_Rkmessage^.rkt)), _IsConsole);
              end
              else begin
                KafkaWriteStatus('Received from topic: ' + Copy(String(PChar(_Rkmessage^.rkt)), 1, _MaxConsumerMessageSize), _IsConsole);
              end;
            ...
            if _MaxConsumerMessageSize = 0 then begin
              ...
              KafkaWriteStatus(Format('%s Message (topic %s, part %d, offset %d, %d bytes):',
                                     [FormatDateTime('yyyy-mm-dd hh:nn:ss.zzz', Now, _FormatSettings),
                                      Copy(String(rd_kafka_topic_name(_Rkmessage^.rkt)), 1, _MaxConsumerMessageSize),
                                       _Rkmessage^.partion,
                                       _Rkmessage^.offset,
                                       _Rkmessage^.len
                                     ]), _IsConsole);
            ...
            if _OnKafkaMessageReceived <> nil then begin
              ...
              if _MaxConsumerMessageSize = 0 then begin
                ...
                _OnKafkaMessageReceived(Copy(MyMessage, 1, _MaxConsumerMessageSize), MyKey, _Rkmessage);
              end;
            end
            else begin
              if _MaxConsumerMessageSize = 0 then begin
...
begin
  _MaxConsumerMessageSize := InMaxMessageSize;
