...
type
  ...
  TProc_Consume = procedure (var rkmessage: Trd_kafka_message_t; opaque: Pointer);

  PProc_dr_msg_cb = ^TProc_dr_msg_cb;
  TProc_dr_msg_cb = procedure(var rk: Trd_kafka_t; var rkmessage: Trd_kafka_message_t; opaque: Pointer);

  PProc_rebalance_cb = ^TProc_rebalance_cb;
  TProc_rebalance_cb = procedure(var rk: Trd_kafka_t; err: Trd_kafka_resp_err_t; var partitions: Trd_kafka_topic_partition_list_t; opaque: Pointer);

  PProc_offset_commit_cb = ^TProc_offset_commit_cb;
  TProc_offset_commit_cb = procedure(var rk: Trd_kafka_t; err: Trd_kafka_resp_err_t; var offsets: Trd_kafka_topic_partition_list_t; opaque: Pointer);

  PProc_error_cb = ^TProc_error_cb;
  TProc_error_cb = procedure(var rk: Trd_kafka_t; err: Int32; reason: PChar; opaque: Pointer);

  PProc_throttle_cb = ^TProc_throttle_cb;
  TProc_throttle_cb = procedure (var rk: Trd_kafka_t;broker_name: PChar;broker_id: Int32;throttle_time_ms: Int32;opaque: Pointer);

  PProc_log_cb = ^TProc_log_cb;
  TProc_log_cb = procedure(var rk: Trd_kafka_t;level: Int32;fac: PChar;buf: PChar);

  PFunc_stats_cb = ^TFunc_stats_cb;
  TFunc_stats_cb = function(var rk: Trd_kafka_t;json: PChar;json_len: size_t;opaque: Pointer): Int32;

  PFunc_socket_cb = ^TFunc_socket_cb;
  TFunc_socket_cb = Function (domain: int32 ; _Type: int32; protocol: int32; opaque: Pointer): Int32;

  PFunc_connect_cb = ^TFunc_connect_cb;
  TFunc_connect_cb = Function (sockfd: Int32;var addr: TSockAddr; addrlen: Int32;id: PChar;opaque: Pointer): Int32;

  PFunc_closesocket_cb = ^TFunc_closesocket_cb;
  TFunc_closesocket_cb = Function(sockfd: Int32; opaque: Pointer): Int32;


  Tmode_t = Int32;
  PFunc_open_cb = ^TFunc_open_cb;
  TFunc_open_cb = Function(pathname: PChar;flags: Int32;mode: Tmode_t;opaque: Pointer): Int32;

  PFunc_partitioner = ^TFunc_partitioner;
	TFunc_partitioner = Function(var rkt: Trd_kafka_topic_t;keydata: PChar;keylen: size_t;partition_cnt: int32;rkt_opaque: Pointer;msg_opaque: Pointer): Int32;

  PProc_consume_cb = ^TProc_consume_cb;
  TProc_consume_cb = procedure(rkmessage: Prd_kafka_message_t; opaque: Pointer);

  PProc_cb = ^TProc_cb;
  TProc_cb = procedure(rk: Prd_kafka_t; err: Trd_kafka_resp_err_t; offsets: Prd_kafka_topic_partition_list_t; opaque: Pointer);

  PProc_logger = ^TProc_logger;
  TProc_logger = function(rk: Prd_kafka_t; level: Int32; fac: PChar; buf: PChar): Pointer;
...
procedure rd_kafka_conf_set_dr_msg_cb(conf: Prd_kafka_conf_t; dr_msg_cb: PProc_dr_msg_cb); cdecl; EXTERNAL RD_EXPORT;
