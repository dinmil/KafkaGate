...
type
  ...
  public
    ...
    constructor Create(InLogNamePrefix: TFavString;       // this is prefix of log file
                       InContext: pointer;                // context is created when application starts
...
constructor TLogThreadForwarder.Create(InLogNamePrefix: TFavString;
  InContext: pointer; InThreadNo: Integer; InLogDirectory: TFavString;
  InQueue: TFavQueue; InSettings: T0MQSetup);
begin
  _BFinished := False;
  _PLogQueue := InQueue;
  _ThreadNo := InThreadNo;
  _Settings := InSettings;
  _LogNamePrefix := InLogNamePrefix;
  ...
  // create log files
  _ProtoLog := TFavProto.Create(_LogDirectory + _LogNamePrefix + '.log', 1024 * 1024 * 10);
  _ProtoErr := TFavProto.Create(_LogDirectory + _LogNamePrefix + '.err', 1024 * 1024 * 10);
